...
function getGalleryList($post){
	...
	$sql = mysql_query("SELECT * FROM `tbl_gallery_records` ORDER BY `added_date` DESC $str");
	$json['gallery_list'] = array();
	if(mysql_num_rows($sql) > 0){
		$json = array("status"=>1, "statusCode"=>1, "msg" => mysql_num_rows($sql).' '.RECORD_FOUND);	
		while($data = mysql_fetch_assoc($sql)){
		$json['gallery_list'][] = array_merge($data,array("gallery_image" => BASE_URLD.'gallery_images/'.$data['gallery_image'], "added_date" => date("d M Y", strtotime($data['added_date'])).' at '.date("H:i A", strtotime($data['added_date']))));
		}
	}else{
		$json = array("status"=>1, "statusCode"=>0, "msg" => NO_RECORD_FOUND);
	}
...
$checkToken = checkTokenKey();
	if($checkToken){
		...
		if(@$post['user_id']!=''){ $user_id = $post['user_id'];}
		if(@$post['gallery_name']!=''){ $gallery_name = $post['gallery_name'];}		
		if($user_id == ''){
			$json = array("status"=>0, "statusCode"=>0, "msg" => "User Id Should Not be Blank!");	
		}elseif($_FILES["gallery_image"] == ''){
			$json = array("status"=>0, "statusCode"=>0, "msg" => "Gallery Image Should Not be Blank!");	
		}else{
			$gallery_image = '';
			if($_FILES['gallery_image']){
				move_uploaded_file($_FILES["gallery_image"]["tmp_name"],
				"gallery_images/" . $_FILES["gallery_image"]["name"]);
				$gallery_image = $_FILES["gallery_image"]["name"];
			}else{
				$gallery_image = '';
			}
			$sql = mysql_query("INSERT INTO `tbl_gallery_records` (`gallery_name`, `gallery_image`, `user_id`,`added_date`) VALUES ('".$gallery_name."','".$gallery_image."','".$user_id."','".date("Y-m-d H:i:s")."')");
			if($sql){
				$json = array("status"=>1, "statusCode"=>1, "msg" => "Your gallery successfully Uploaded!");
			}else{
				$json = array("status"=>1, "statusCode"=>0, "msg" => "Gallery Not Added!");
			}
		}	
	}else{
		$json = array("status"=>0, "statusCode"=>0, "msg" => INVALID_TOKEN );
...
//$msg = array("to" => getUserName($receiver_id), "notification" => array("body" => $message,"title" => $title),"data" => array("sender_id" => $sender_id, "receiver_id" => $receiver_id));
$msg = array("notification" => array("body" => $message,"title" => $title,"icon" => "myicon"));
