				...
				<table class="table datatable-html">
					...
					<?php $a = 0; foreach($alldata as $data){ $a++; ?>
						<tr id="row_<?=$data->product_id;?>">
							...
							<td><?php if($data->category_id!=0){ echo ucfirst(getCategoryName($data->category_id)); }?></td>
							<td><?php if($data->sub_category_id!=0){ echo ucfirst(getCategoryName($data->sub_category_id)); }?></td>
						   <td><?php if($data->user_id!=0){ echo ucfirst(getUserName($data->user_id)); } ?></td>
						   <td class="td-exp" style="width: 110px;"><?php echo $data->location;?></td>					              
							<td class="td-exp" style="width: 110px;"><?php echo $data->added_date;?></td>
							<td id="upbtn_<?=$data->product_id;?>"><?php if($data->status == '1'){ echo '<span title="Click to Inactive" class="label label-success" onclick="updateData('.$data->product_id.',0)">Active</span>'; }else{ echo '<span title="Click to Active" class="label label-default"  onclick="updateData('.$data->product_id.',1)">Inactive</span>'; }?></td>
							<td class="text-center">
								<ul class="icons-list">
									<li><a href="<?php echo site_url("Dashboard/add_offer").'/'.$data->product_id;?>" ><i class="icon-add"></i></a>&nbsp;&nbsp;&nbsp;<a href="<?php echo site_url("Dashboard/edit_product").'/'.$data->product_id;?>" ><i class="icon-pencil7"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onClick="deleteData(<?=$data->product_id;?>)"><i class="icon-trash-alt"></i></a></li>
								</ul>
							</td>
...
<script>
	function deleteData(rid){
			...
			if(conf == true){
				var action = 'delete';
				var tblname = 'tbl_product_records';
				var key = 'product_id';
				$.ajax({
			type: "POST",
